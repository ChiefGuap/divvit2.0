-- Migration 019: Manual split awards zero points

-- Update handle_bill_completed to check for manual split
...
BEGIN
  ...
  -- Skip points if this was a manual split
  IF NEW.details->>'is_manual' = 'true' THEN
    RETURN NEW;
  END IF;
...
-- Update handle_group_photo to check for manual split
